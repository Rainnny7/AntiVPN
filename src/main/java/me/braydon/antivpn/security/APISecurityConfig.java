...
public class APISecurityConfig {
    ...
    @Autowired
    public APISecurityConfig(@NonNull APIKeyRepository repository) {
    ...
    public SecurityFilterChain filterChain(@NonNull HttpSecurity http) throws Exception {
        ...
        if (repository.count() == 0) {
            APIKey apiKey = new APIKey(UUID.randomUUID().toString(), 0, null, new Date());
            repository.save(apiKey);
            
            // Log the creation
            log.info("-" .repeat(35));
            log.info("Default API key created: " + apiKey.getKey());
            log.info("-" .repeat(35));
        }
        http.csrf().disable()
